...
-- Políticas abiertas para etapa MVP real. Más adelante las endurecemos con auth.uid().
do $$
begin
  begin create policy "usuarios_all_v20" on usuarios for all using (true) with check (true); exception when duplicate_object then null; end;
  begin create policy "choferes_all_v20" on choferes for all using (true) with check (true); exception when duplicate_object then null; end;
  begin create policy "acompanantes_all_v20" on acompanantes for all using (true) with check (true); exception when duplicate_object then null; end;
end $$;
